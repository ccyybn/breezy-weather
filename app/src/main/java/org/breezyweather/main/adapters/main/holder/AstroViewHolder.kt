...
class AstroViewHolder(parent: ViewGroup) : AbstractMainCardViewHolder(
    ...
    override fun onBindView(
        ...
        mWeather?.today?.sun?.let { sun ->
            if (sun.isValid) {
                var sunriseTime = sun.riseDate!!.getFormattedTime(location, context, context.is12Hour)
                var sunsetTime = sun.setDate!!.getFormattedTime(location, context, context.is12Hour)
        ...
        mWeather?.today?.moon?.let { moon ->
            if (moon.isValid) {
                var moonriseTime = moon.riseDate!!.getFormattedTime(location, context, context.is12Hour)
                var moonsetTime = moon.setDate!!.getFormattedTime(location, context, context.is12Hour)
    ...
    private fun ensureTime(weather: Weather, timeZone: TimeZone) {
        ...
        if (weather.today?.sun?.riseDate != null && weather.today!!.sun!!.setDate != null) {
            if (weather.today?.sun?.riseDatePre != null && weather.today?.sun?.setDatePre != null && currentTime < weather.today?.sun?.setDatePre!!.time) {
                mStartTimes[1] = weather.today!!.sun!!.riseDatePre!!.time
                mEndTimes[1] = weather.today!!.sun!!.setDatePre!!.time
            } else {
